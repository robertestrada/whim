...
const FeedFilterRating = ({ idx, setPageData, lastFilterTerm, setLastFilterTerm }) => {

  const handleRadioClick = () => {
    setPageData({ "page": 1, "loadMore": false, "tab": "search" });
    setLastFilterTerm({ ...lastFilterTerm, 'rating': idx });
  };

  return (
    <div key={idx} className="filter__rating-option" onClick={() => handleRadioClick()}>
      <div className={ lastFilterTerm.rating === idx ? "filter__rating-radio radio-selected" : "filter__rating-radio" }>
        <div className={ lastFilterTerm.rating === idx ? "filter__rating-dot dot-selected" : "filter__rating-dot" }></div>
      </div>
