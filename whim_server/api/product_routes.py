...
def get_popular_products(page):
  results = Product.query.order_by(desc(Product.updated_at)).paginate(page, 18, False)
...
def get_express_products(page):
  results = Product.query.filter(Product.shipping_speed > 0).order_by(desc(Product.updated_at)).paginate(page, 18, False)
  more_data = results.has_next
  products = results.items
  data = [product.feed_dict() for product in products]
  return {"data": data, "more_data": more_data}, 200
...
def get_category_products(category, page):
  results = Product.query.filter(Product.category==category).order_by(desc(Product.updated_at)).paginate(page, 18, False)
  more_data = results.has_next
  products = results.items
  data = [product.feed_dict() for product in products]
  return {"data": data, "more_data": more_data}, 200
